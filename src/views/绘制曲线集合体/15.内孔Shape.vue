...
export default {
    ...
    methods: {
        ...
        createGeometry: function () {
            ...
            shape.lineTo(0, 50) //从0,0 朝向0,50画直线（每次执行lineTo方法都会改变currentPoint的值）
            shape.lineTo(50, 50) //从0,50 朝向50,50画直线
            shape.lineTo(50, 0)
            const curve = new THREE.Shape()
            curve.moveTo(20, 20)
            curve.arc(0, 0, 10, 0, Math.PI * 2)
            shape.holes.push(curve)

            const extrudeGeometry = new THREE.ExtrudeGeometry(shape, { depth: 20 })
