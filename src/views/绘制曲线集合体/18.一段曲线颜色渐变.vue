...
export default {
    ...
    methods: {
        ...
        createGeometry: function () {
            ...
            const color = []
            geometry.setFromPoints(catmullRomCurve3.getPoints(30))
            const points = geometry.attributes.position.count - 1;
            const color1 = new THREE.Color(0xffff00) //黄色
            const color2 = new THREE.Color(0xffffff) //黄色
            for (var i = 0; i < points; i++) {
                const percent = i / points
                const col = new THREE.Color()
                const c = col.lerpColors(color1, color2, percent) //颜色混合
