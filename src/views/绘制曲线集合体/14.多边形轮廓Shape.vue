...
export default {
    ...
    methods: {
        ...
        createGeometry: function () {
            ...
            console.log('Shape的默认坐标原点是0,0', shape.currentPoint)
            shape.moveTo(5, 0)
            shape.lineTo(5, 50) //从0,0 朝向0,50画直线（每次执行lineTo方法都会改变currentPoint的值）
            shape.lineTo(50, 50) //从0,50 朝向50,50画直线
            shape.lineTo(50, 0)
            const shappeGeometry = new THREE.ShapeGeometry(shape)
            const material = new THREE.MeshLambertMaterial()
            material.side = THREE.DoubleSide
            material.color.set('green')
            const mesh = new THREE.Mesh(shappeGeometry, material)
            this.scene.add(mesh)
        },
        createGeometry1: function () {
            const shape = new THREE.Shape()
            shape.moveTo(-5, 0)
            shape.lineTo(-50, 0) //从0,0 到150，0 绘制直线
            shape.arc(0, 0, 50, Math.PI, Math.PI / 2, true)
            shape.lineTo(-5, 50)
            const shappeGeometry = new THREE.ShapeGeometry(shape)
            const material = new THREE.MeshLambertMaterial()
            material.side = THREE.DoubleSide
        ...
        createGeometry2: function () {
            ...
            shape.moveTo(-5, 0)
            shape.lineTo(-50, 0) //从0,0 到150，0 绘制直线
            shape.absarc(-50, 0, 50, Math.PI, Math.PI / 2, true)
            shape.lineTo(-5, 50)
            const shappeGeometry = new THREE.ShapeGeometry(shape)
            const material = new THREE.MeshLambertMaterial()
            material.side = THREE.DoubleSide
